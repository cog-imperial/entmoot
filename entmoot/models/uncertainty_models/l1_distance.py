...
class L1Distance(NonCatDistance):
    ...
    def get_gurobipy_model_constr_terms(self, model):
        ...
        aux_pos = model.addVars(len(self.x_trafo), feat_dict,
                                name="aux_pos",
                                vtype='C')

        aux_neg = model.addVars(len(self.x_trafo), feat_dict,
                                name="aux_neg",
                                vtype='C')
        ...
        for data_idx, xi in enumerate(self.x_trafo):
            for i, idx in enumerate(self._problem_config.non_cat_idx):
                ...
                model.addSOS(GRB.SOS_TYPE1, [aux_pos[data_idx, idx], aux_neg[data_idx, idx]])
